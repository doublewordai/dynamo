...
<ParamField path="DYN_EVENT_PLANE_HOST" type="string" default="auto-detected local address">
  ...
  invalid, including IPv4-mapped wildcards such as `::ffff:0.0.0.0`. When unset, automatic
  advertisement stays in the IPv4 bind family and, on Unix, excludes down interfaces. If no eligible
  interface has a usable non-loopback IPv4 address, automatic selection advertises IPv4 loopback
  (`127.0.0.1` when no IPv4 loopback address is found), and remote subscribers cannot reach the publisher.

  This variable changes the discovery address, not the listener address. Direct ZMQ continues to bind
  an IPv4 wildcard. For remote subscribers, an explicit override must identify a reachable IPv4
  address served by that listener. An IPv6 value does not enable IPv6 listening. This variable does
  not affect NATS or ZMQ broker mode.
</ParamField>
...
<ParamField path="DYN_DYNAMO_REQUEST_QUEUE_LIMIT" type="integer" default="40000">
  ...
  across every endpoint the process serves. Set to `0` to disable queueing at the backend admission
  gate: requests are rejected as queue full when the engine-request limit is reached. Unset or
  invalid values, including negative integers, retain the default.
</ParamField>
...
<ParamField path="DYN_DYNAMO_REQUEST_QUEUE_ENABLE_CONTROLLED_DELAY" type="boolean" default="false">
  ...
  there is no command-line flag. An unset, empty or unrecognized value leaves it disabled.
</ParamField>

<ParamField path="DYN_DYNAMO_REQUEST_QUEUE_ENABLE_ADAPTIVE_LIFO" type="boolean" default="false">
