...
<ParamField path="dynamo_backend_admission_rejection_total" type="counter">
  Cumulative requests the gate refused, labelled `reason`: `queue_full` when the limit was busy and the queue was full, `request_expired` when the request exceeded its permitted queue residence, `evicted` when a higher-priority arrival evicted it at the engine-queue margin. A cancelled request is not a rejection and is counted under none of them.
</ParamField>
