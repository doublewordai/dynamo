...
- `--router-reported-load`: Score each candidate rank by its expected time to first token in seconds instead of the block-based cost. The wait is the larger of the router's tracked prefill backlog for the rank and the rank's reported engine queue (`--router-reported-queue-tokens-per-request` tokens per waiting request), plus this request's uncached prompt tokens, divided by `--router-prefill-rate-tokens-per-sec`; a rank whose reported KV usage is above `--router-kv-headroom-frac` pays a penalty and one that cannot fit the request is avoided. `--router-locality-band-secs` keeps a request on the rank holding the most of its prompt while that rank's wait is within the band of the best rank. Off by default. Use this when requests are long enough that a worker's queue, not its resident KV, decides time to first token.
