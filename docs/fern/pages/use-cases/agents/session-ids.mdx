...
Dynamo accepts and forwards the OpenAI request body `user` field but does not
use it for affinity, agent identity, or tracing identity. Clients that need
session affinity must send `X-Dynamo-Session-ID` or a supported agent-native
session header.
...
<Tabs>
  ...
  <Tab title="Codex">
    Dynamo maps Codex's per-agent thread headers:

    - `thread-id` to `session_id`
    - `x-codex-parent-thread-id` to `parent_session_id` for child threads
