...
use super::affinity_scale_up::{ScaleUpMigrationTracker, ScaleUpSnapshot};
use std::pin::Pin;
...
struct Inner {
    ...
    scale_up: OnceLock<ScaleUpMigrationTracker>,
...
impl Hold {
    ...
    pub fn migration_worker_ids(&self) -> Option<&std::collections::HashSet<u64>> {
        match self {
...
impl SessionAffinity {
    ...
    pub fn enable_scale_up(&self, tracker: ScaleUpMigrationTracker) -> bool {
        self.inner.scale_up.set(tracker).is_ok()
    }
            ...
            Entry::Occupied(mut entry) => match entry.get_mut() {
                ...
                AffinityEntry::Bound {
                    ...
                    if requested_target.is_none()
                        && *active_leases == 0
                        && let (Some(tracker), Some(previous)) =
                            (self.inner.scale_up.get(), scale_snapshot.as_ref())
                    {
                        let evaluation = tracker.evaluate(session_id, previous);
                        if let Some(migration_workers) = evaluation.migration_workers {
                            let migration = AffinityMigration {
                                coordinator: Arc::downgrade(&self.inner),
                                session_id: session_id.to_string(),
                                ...
                                old_target: *target,
                                old_version: *version,
                                old_migration_generation: *migration_generation,
                                previous_snapshot: previous.clone(),
                                next_snapshot: evaluation.snapshot,
                                migration_workers,
                                active: true,
                            };
                            *entry.get_mut() = AffinityEntry::Migrating {
                                old_target: migration.old_target,
                                revision: migration.revision,
                                notify: migration.notify.clone(),
                            };
                            return Ok(AcquireStep::Held(Hold::Migrate(migration)));
                        }
    ...
    pub fn commit(
        ...
        match hold {
            ...
            Hold::Migrate(migration) => migration.commit(dispatched),
    ...
    pub fn query_target(
        ...
        if let AffinityEntry::Migrating { old_target, .. } = entry.value() {
            validate_bound_target(session_id, *old_target, requested_target)?;
            return Ok(Some(*old_target));
        }
    ...
    pub fn apply_replica_update(
        ...
        self.inner
            .apply_replica_update(session_id, target, version, 0)
    }

    /// Apply a replicated scale-up binding, retaining its migration generation.
    pub fn apply_migration_update(
        &self,
        session_id: String,
        target: AffinityTarget,
        version: AffinityVersion,
        ...
        self.inner
            .apply_replica_update(session_id, target, version, generation)
    }
...
impl SessionAffinity {
    ...
    pub fn lease_count(&self, session_id: &str) -> Option<usize> {
        ...
        Some(match entry.value() {
            AffinityEntry::Initializing { .. } | AffinityEntry::Migrating { .. } => 0,
...
impl Inner {
            ...
            let generation = self
                ...
                .and_then(|entry| match entry.value() {
                    AffinityEntry::Bound {
                        ...
                        Some(*migration_generation)
                    }
                    _ => None,
                });
            if let Some(generation) = generation {
                replica.publish_migration(session_id, target, version, generation);
            }
        }
    }
    ...
    fn apply_replica_update(
        ...
        let now = Instant::now();
        let fresh = || AffinityEntry::Bound {
            target,
            revision: self.next_revision.fetch_add(1, Ordering::Relaxed),
            version,
            active_leases: 0,
            idle_deadline: now + self.ttl,
            scale_snapshot: self.scale_up.get().map(ScaleUpMigrationTracker::snapshot),
        ...
        match self.entries.entry(session_id) {
            ...
            Entry::Occupied(mut entry) => match entry.get_mut() {
                ...
                AffinityEntry::Bound {
                    ...
                    scale_snapshot,
                    ..
                } if (generation, version) >= (*migration_generation, *existing_version) => {
                    let outcome = if *existing == target {
                        ReplicaApplyOutcome::Refreshed
                    } else if generation > *migration_generation {
                        ReplicaApplyOutcome::ReboundMigration
                    } else if version > *existing_version {
                        ReplicaApplyOutcome::ReplacedNewer
                    } else {
                        return ReplicaApplyOutcome::IgnoredConflict;
                    };
                    // A replicated migration has already evaluated the current topology.
                    if generation > *migration_generation {
                        *scale_snapshot =
                            self.scale_up.get().map(ScaleUpMigrationTracker::snapshot);
                    }
                    *existing = target;
...
impl AffinityInitialization {
    ...
    pub fn commit(mut self, target: AffinityTarget) -> Result<AffinityLease, AffinityError> {
        ...
        *entry = AffinityEntry::Bound {
            ...
            scale_snapshot: inner.scale_up.get().map(ScaleUpMigrationTracker::snapshot),
            migration_generation: 0,
        };
        drop(entry);
        self.active = false;
...
pub struct AffinityMigration {
    ...
    migration_workers: Arc<std::collections::HashSet<u64>>,
    active: bool,
}

impl AffinityMigration {
    fn commit(mut self, target: AffinityTarget) -> Result<AffinityLease, AffinityError> {
        ...
        let Some(mut entry) = inner.entries.get_mut(&self.session_id) else {
            return Err(AffinityError::InvalidArgument(
                "session affinity migration was cancelled".to_string(),
            ));
        };
        if !matches!(
            entry.value(),
            ...
            return Err(AffinityError::InvalidArgument(
                "session affinity migration changed".to_string(),
            ));
        }
...
mod tests {
    ...
    fn initialize(table: &SessionAffinity) -> AffinityInitialization {
        match table.try_acquire("s", None).expect("acquire") {
            ...
            AcquireStep::Held(Hold::Bound { .. } | Hold::Migrate(_)) => {
                panic!("session is already bound")
            }
    ...
    #[tokio::test]
    async fn scale_up_migration_rolls_back_until_dispatch_commits() {
        let table = table();
        let initial = ScaleUpSnapshot::from_workers(0, &[(7, 100)]);
        let tracker = ScaleUpMigrationTracker::for_test("model", initial.clone());
        table.enable_scale_up(tracker.clone());
        let next = ScaleUpSnapshot::from_workers(1, &[(7, 100), (8, 100)]);
        let session = (0..1000)
            .map(|n| format!("session-{n}"))
            .find(|id| {
                tracker.set_snapshot_for_test(next.clone());
                let selected = tracker.evaluate(id, &initial).migration_workers.is_some();
                tracker.set_snapshot_for_test(initial.clone());
                selected
            })
            .expect("capacity-proportional cohort has sessions");
        ...
        tracker.set_snapshot_for_test(next);
        let AcquireStep::Held(hold) = table.try_acquire(&session, None).unwrap() else {
            panic!("bound")
        };
        assert!(
            matches!(hold, Hold::Bound { .. }),
            "active requests prevent migration"
        );
        drop(hold);
        drop(lease);
        let AcquireStep::Held(hold) = table.try_acquire(&session, None).unwrap() else {
        ...
        assert_eq!(
            hold.migration_worker_ids().unwrap(),
            &std::collections::HashSet::from([8])
        );
        drop(hold); // Selection/dispatch failure must leave the original pin intact.
        assert_eq!(table.query_target(&session, None).unwrap(), Some(target));
        let AcquireStep::Held(hold) = table.try_acquire(&session, None).unwrap() else {
            panic!("retry migration")
        };
        let migrated = AffinityTarget::new(8, Some(0));
        drop(table.commit(hold, migrated).unwrap());
        assert_eq!(table.query_target(&session, None).unwrap(), Some(migrated));
        let AcquireStep::Held(hold) = table.try_acquire(&session, None).unwrap() else {
            panic!("bound")
        };
        assert!(
            matches!(hold, Hold::Bound { .. }),
            "migration evaluated once per topology"
        );
    }

    #[tokio::test]
    async fn migration_replica_update_cannot_be_undone_by_old_heartbeat() {
        let table = table();
        let old = AffinityTarget::new(7, Some(0));
        let new = AffinityTarget::new(8, Some(0));
        let version = AffinityVersion {
            sequence: 1,
            writer_id: 1,
        };
        assert_eq!(
            table.apply_replica_update("s".into(), old, version),
            ReplicaApplyOutcome::Inserted
        );
        assert_eq!(
            table.apply_migration_update("s".into(), new, version, 1),
            ReplicaApplyOutcome::ReboundMigration
        );
        assert_eq!(
        ...
        assert_eq!(table.query_target("s", None).unwrap(), Some(new));
    }
