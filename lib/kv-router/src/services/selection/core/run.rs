...
impl SelectionCore {
    ...
    async fn run_selection_inner(
        ...
        let mut allowed_worker_ids = match self.host.eligibility.lora_worker_filter.as_deref() {
        ...
        if let Some(migration_workers) = affinity_hold.as_ref().and_then(Hold::migration_worker_ids)
        {
            match &mut allowed_worker_ids {
                Some(allowed) => allowed.retain(|worker| migration_workers.contains(worker)),
                None => allowed_worker_ids = Some(migration_workers.clone()),
            }
        }
