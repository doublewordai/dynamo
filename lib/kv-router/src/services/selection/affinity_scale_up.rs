...
impl ScaleUpSnapshot {
    ...
    pub fn from_workers(generation: u64, workers: &[(WorkerId, u128)]) -> Arc<Self> {
...
pub struct ScaleUpEvaluation {
    pub snapshot: Arc<ScaleUpSnapshot>,
    pub migration_workers: Option<Arc<HashSet<WorkerId>>>,
}

#[derive(Clone)]
pub struct ScaleUpMigrationTracker {
    scope: Arc<str>,
...
impl ScaleUpMigrationTracker {
        ...
        C: WorkerConfigLike + Clone + Send + Sync + 'static,
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if !has_baseline {
                    *background.current.write() = Arc::new(ScaleUpSnapshot::new(0, workers));
                    has_baseline = true;
                    continue;
                }
    ...
    pub fn snapshot(&self) -> Arc<ScaleUpSnapshot> {
        self.current.read().clone()
    }

    #[cfg(any(test, feature = "testing"))]
    pub fn for_test(scope: &str, current: Arc<ScaleUpSnapshot>) -> Self {
        Self {
            scope: Arc::from(scope),
    ...
    #[cfg(any(test, feature = "testing"))]
    pub fn set_snapshot_for_test(&self, snapshot: Arc<ScaleUpSnapshot>) {
...
fn worker_capacities<C: WorkerConfigLike>(
    configs: &std::collections::HashMap<WorkerId, C>,
