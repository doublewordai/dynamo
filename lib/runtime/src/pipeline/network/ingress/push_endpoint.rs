...
pub struct PushEndpoint {
    ...
    /// Requests accepted and not yet finished, shared with the server that
...
impl PushEndpoint {
    ...
    pub async fn start(
        ...
        loop {
            ...
            if let Some(req) = req {
                // Count the request from acceptance: the acknowledgement below
                // can block under backpressure, and an accepted request not yet
                // counted would be invisible to a drain. Every path from here
                // reaches the spawned task, which decrements.
