...
impl TcpStreamServer {
    ...
    pub async fn new_with_resolver<R: IpResolver>(
        ...
        let resolved_host = if let Some(host_or_interface) =
            options.host.as_deref().or(options.interface.as_deref())
        {
        ...
        let port = advertise_port_override.unwrap_or(local_address.port());
        let host = advertise_host.unwrap_or_else(|| resolved_host.advertise_ip().to_string());
        let address = match host.parse::<IpAddr>() {
            Ok(ip) => SocketAddr::new(ip, port).to_string(),
            Err(_) => format!("{host}:{port}"),
        };
        tracing::debug!(%local_address, %address, "tcp transport service started");
        Ok(Arc::new(Self {
            local_address,
            address,
            state,
