...
fn build_transport_type_inner(
    ...
    match mode {
        RequestPlaneMode::Tcp => {
            ...
            let host = std::env::var("DYN_TCP_RPC_ADVERTISE_HOST")
                .unwrap_or_else(|_| address.ip().to_string());
            let port = std::env::var("DYN_TCP_RPC_ADVERTISE_PORT")
                .ok()
                .and_then(|p| p.parse::<u16>().ok())
                .filter(|&p| p != 0)
                .unwrap_or(address.port());
            let advertised = match host.parse::<std::net::IpAddr>() {
                Ok(ip) => SocketAddr::new(ip, port).to_string(),
                Err(_) => format!("{host}:{port}"),
            };
            Ok(tcp_transport_type(advertised, endpoint_id, connection_id))
        }
...
mod tests {
    ...
    fn tcp_transport_uses_concrete_ipv4_and_bracketed_ipv6_addresses() {
        ...
        for (address, expected) in [
            ...
            let transport = tcp_transport_type(address, &endpoint_id, 0x2a);
