...
pub(crate) fn configured_family(
    ...
    match (tool_call_parser, reasoning_parser) {
        ...
        (Some(tool), Some(reasoning)) => {
            let family = unified_family(tool)?;
            (unified_family(reasoning) == Some(family)).then_some(family)
        }
        _ => None,
    }
}
...
pub(crate) fn selected_family(
    ...
    configured.filter(|family| match *family {
        ...
        | MIMO_UNIFIED_FAMILY
        | MUSE_GLIMMER_UNIFIED_FAMILY => true,
        _ => false,
    })
}
...
fn detect_prefill(family: &str, content: &str) -> anyhow::Result<UnifiedParserStartingState> {
    match family {
        HUNYUAN_UNIFIED_FAMILY => Ok(super::hunyuan_parser::detect_starting_state(content)),
        MIMO_UNIFIED_FAMILY => Ok(super::mimo_parser::detect_starting_state(content)),
        MUSE_GLIMMER_UNIFIED_FAMILY => Ok(UnifiedParserStartingState::None),
...
impl ChoiceState {
    ...
    pub(crate) fn new_default(family: &str, tools: &[Tool]) -> anyhow::Result<Self> {
        Ok(Self {
            family: family.to_string(),
            parser: create_parser(family, tools)?,
            opened_calls: HashSet::new(),
            tool_emitted: false,
            failed: false,
        })
