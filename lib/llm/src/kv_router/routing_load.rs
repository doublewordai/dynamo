...
impl RoutingLoadContext {
    ...
    pub(crate) async fn start_for_model(
        client: Client,
        source: RouterLoadSource,
        thresholds: LoadThresholdHandle,
        parent_token: &CancellationToken,
        ...
        let (scheduler_load, monitor) = if source.monitors_sequence_load() {
            ...
            let monitor = match model_name {
                Some(name) => monitor.with_model_name(name),
                None => monitor,
            };
            let monitor = monitor.with_remote_block_metrics(publish_remote_blocks);
