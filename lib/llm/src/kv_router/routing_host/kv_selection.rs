...
impl RoutingHost {
    async fn select_best_match(&self, args: BestMatchArgs<'_>) -> Result<SelectionOutcome, Error> {
        // Queue snapshots are only useful for immediate selection. A queued request
        // must remain eligible for workers that free up after it was enqueued.
        if args.pinned_worker.is_some() || self.kv_router().queueing_enabled() {
            return self.select_best_match_inner(args).await;
        }
        let saturated: HashSet<_> = self
            .inner
            ...
            .filter(|worker| {
                args.allowed_worker_ids
                    .as_ref()
                    .is_none_or(|allowed| allowed.contains(worker))
            })
            .filter(|worker| {
                args.excluded_worker_ids
                    .as_ref()
                    .is_none_or(|excluded| !excluded.contains(worker))
            })
            .collect();
        if saturated.is_empty()
            || args
                .allowed_worker_ids
                .as_ref()
                .is_some_and(|allowed| allowed.iter().all(|worker| saturated.contains(worker)))
        {
            return self.select_best_match_inner(args).await;
        }
        let mut steered = args.clone();
        steered
            .excluded_worker_ids
            .get_or_insert_with(HashSet::new)
            .extend(saturated);
        match self.select_best_match_inner(steered).await {
            Ok(selection) => {
            ...
            {
                // The dispatch gate remains authoritative when no alternative can serve.
                self.select_best_match_inner(args).await
            }
            Err(error) => Err(error),
        }
    }

    async fn select_best_match_inner(
        &self,
        args: BestMatchArgs<'_>,
    ...
    pub(super) async fn select_worker_outcome(
        ...
        if let Some(migration_workers) = migration_worker_ids {
            match &mut allowed_worker_ids {
                Some(allowed) => allowed.retain(|worker| migration_workers.contains(worker)),
                None => allowed_worker_ids = Some(migration_workers),
            }
        }
