...
use super::*;
use crate::kv_router::publisher::WorkerLoadReport;
...
impl PoolConfig {
    fn validate(&self) -> anyhow::Result<()> {
        ...
        for (name, pool) in &self.pools {
            ...
            anyhow::ensure!(
                pool.cap > 0 && pool.minimum > 0,
                "pool caps and minima must be positive"
            );
        }
        anyhow::ensure!(
...
struct View {
    ...
    healthy: bool,
    occupied: u64,
    reported_occupied: u64,
    local_occupied: usize,
    effective_cap: u64,
    engine_cap: u64,
    waiting: u64,
    kv_used_blocks: u64,
    ...
    fn view(&self, id: u64, config: &PoolConfig, now: Instant) -> View {
        ...
        View {
            ...
            eligible_ranks: ranks
                .filter(|rank| self.rank_occupied(*rank) < self.rank_cap)
                .collect(),
        }
    }
}
...
impl State {
    ...
    fn reconcile(&mut self, configs: &HashMap<u64, ModelRuntimeConfig>, now: Instant) {
        ...
        for (id, config) in discovered {
                ...
                self.members.insert(
                    *id,
                    Member {
                        ...
                        rank_cap: config.max_num_seqs.unwrap_or(0),
            ...
            if let Some(member) = self.members.get_mut(id) {
                member.rank_cap = config.max_num_seqs.unwrap_or(0);
    ...
    fn report(&mut self, event: WorkerLoadReport, now: Instant) {
        let load = event.load;
        ...
        let (Some(active), Some(revision)) = (event.num_active_reqs, load.load_report_revision)
        else {
            return;
        };
        if member
...
mod tests {
    ...
    fn state() -> (State, Instant) {
        let config: PoolConfig = serde_json::from_value(serde_json::json!({
            ...
            "pools": {"interactive": {"cap": 2, "minimum": 1}, "throughput": {"cap": 8, "minimum": 1}},
            "default_pool": "throughput",
            "sustained_seconds": 4, "cooldown_seconds": 5
        })).unwrap();
        config.validate().unwrap();
        let now = Instant::now();
        let mut state = State::new(config);
        for (id, name, pool) in [
            (1, "worker-a", "interactive"),
            (2, "worker-b", "throughput"),
            (3, "worker-c", "throughput"),
        ] {
    ...
    #[test]
    fn interactivity_discovery_balances_joins_without_known_worker_ids() {
        ...
        for id in 1..=6 {
            ...
            assert_eq!(state.members[&id].stable_id, id.to_string());
        }
    ...
    #[test]
    fn interactivity_removal_repairs_imbalance_after_draining() {
        ...
        state.rebalance(now);
        assert_eq!(state.members[&2].pool, "interactive");
        assert_eq!(state.members[&3].pool, "throughput");
    }

    #[test]
    fn interactivity_configs_allow_independent_endpoints_but_reject_duplicates() {
        let config = serde_json::json!({
            "endpoint": "demo.first.generate",
            "pools": {"interactive": {"cap": 2, "minimum": 1}, "throughput": {"cap": 8, "minimum": 1}},
            "default_pool": "throughput"
        });
        let mut second = config.clone();
        second["endpoint"] = "demo.second.generate".into();
        let encode = |v: serde_json::Value| serde_json::to_vec(&v).unwrap();
        assert_eq!(parse_configs(&encode(config.clone())).unwrap().len(), 1);
        assert_eq!(
            parse_configs(&encode(serde_json::json!([config, second])))
                .unwrap()
                .len(),
            2
        );
        assert!(parse_configs(&encode(serde_json::json!([config, config]))).is_err());
        assert!(parse_configs(b"[]").is_err());
    }

    #[test]
    fn interactivity_managers_do_not_share_accounting_or_classification() {
        let (first, now) = state();
        let (mut second, _) = state();
        second.config.endpoint = "pooldemo.worker2.generate".into();
        let first = Arc::new(PoolManager {
            state: Mutex::new(first),
        });
        let second = Arc::new(PoolManager {
            state: Mutex::new(second),
        });
        // Deliberately reuse worker IDs and class names to expose any shared map.
        let a = first
        ...
        let untouched = second.state.lock();
        assert_eq!(untouched.members[&2].pool, "throughput");
        assert!(
            untouched
                .members
                .values()
                .all(|m| m.local.is_empty() && m.target.is_none())
        );
        assert!(untouched.pressure_since.is_empty());
        drop(a);
        drop(b);
    }

    #[test]
    fn worker_budget_spans_dp_ranks_and_releases_locally() {
        let (state, _) = state();
    ...
    #[test]
    fn observation_reconciles_delta_and_heartbeat_does_not() {
        let (mut state, now) = state();
        let member = state.members.get_mut(&1).unwrap();
        member.local.insert(
            1,
            LocalRequest {
                pool: "interactive".into(),
                rank: 0,
                report_revision: 1,
            },
        );
        let report = member.reports.get_mut(&0).unwrap();
        report.active = 1;
        report.received = now;
        assert_eq!(member.rank_occupied(0), 2);
        member.reports.get_mut(&0).unwrap().revision = 2;
        assert_eq!(member.rank_occupied(0), 1);
    }

    #[test]
    fn heartbeat_replay_cannot_refresh_expired_scheduler_observation() {
        let (mut state, now) = state();
        let event = |revision| WorkerLoadReport {
            load: dynamo_kv_router::protocols::ActiveLoad {
                worker_id: 1,
                dp_rank: 0,
                load_report_revision: Some(revision),
                ..Default::default()
            },
            num_active_reqs: Some(0),
        };
        let later = now + Duration::from_secs(6);
        state.report(event(1), later);
        assert_eq!(state.members[&1].reports[&0].received, now);
        assert!(!state.views(later)[0].healthy);
        state.report(event(2), later);
        assert_eq!(state.members[&1].reports[&0].received, later);
        // The other DP rank is still stale, so the entire worker stays excluded.
        assert!(!state.views(later)[0].healthy);
    }

    #[test]
    fn borrowed_class_lowers_worker_wide_cap() {
        let (state, _) = state();
        let manager = Arc::new(PoolManager {
            state: Mutex::new(state),
        });
    ...
    #[test]
    fn missing_or_stale_rank_disables_entire_worker() {
        let (mut state, now) = state();
        assert!(state.views(now)[0].healthy);
        assert!(!state.views(now + Duration::from_secs(6))[0].healthy);
        state.members.get_mut(&1).unwrap().reports.remove(&1);
        let view = &state.views(now)[0];
        assert!(!view.healthy);
        assert_eq!(view.available_by_pool["interactive"], 0);
    }

    #[test]
    fn sustained_pressure_drains_then_reclassifies_without_worker_mutation() {
        let (mut state, now) = state();
        state
            .members
            .get_mut(&1)
        ...
        state.rebalance(later);
        assert_eq!(state.members[&2].pool, "interactive");
        assert_eq!(state.members[&2].rank_count, 2);
        assert_eq!(state.members[&2].rank_cap, 8);
        assert_eq!(state.members[&3].pool, "throughput");
    }
}
