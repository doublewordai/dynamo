...
/// Placement reported by an advisory selection, without admission or booking.
...
impl KvRouter {
    ...
    pub(crate) async fn preview_placement(
        ...
        let session_context =
            session_id.map(|id| dynamo_kv_router::SessionContext::new(id, None, None, None));
        match self
            ...
            .await
        {
            Ok(admitted) => match admitted.outcome {
                FindBestMatchOutcome::Routed {
                    worker,
                    overlap_blocks,
                    logit,
                    ..
                } => Ok(Some(PlacementPreview {
                    worker,
                    overlap_blocks,
                    logit,
                })),
                FindBestMatchOutcome::QueueRejected { .. } => Ok(None),
            },
            Err(error)
