    ...
    Resp: Data + HasTokenIds,
{
        ...
        while self.retries_left > 0 {
                ...
                && (has_no_worker_left(err) || is_admission_rejection(err))
            {
                response_stream = match self
                    .continue_in_other_worker_set(
                        migratable_error_in_chain(err.as_ref())
                            .map_or(ErrorType::Unavailable, DynamoError::error_type),
                    )
                    .await
                {
    ...
    fn correct_replayed_usage(&self, response: &mut Annotated<Resp>) {
        ...
        if let Some(details) = usage.prompt_tokens_details.as_mut() {
            // After replay, the worker's cache count describes the retry prompt.
            // Preserve the logical request's original cache attribution instead.
            details.cached_tokens = self
                .request
                .tracker
                .as_ref()
