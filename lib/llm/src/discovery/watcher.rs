...
use crate::{
    backend::Backend,
    discovery::{
        LoadThresholdHandle, WORKER_TYPE_DECODE, WorkerSet, WorkerSetMigrationFallback,
    ...
    async fn prepare_worker_set(
            ...
            && (card.model_type.supports_chat() || card.model_type.supports_completions())
        {
            ...
            let load_context = if needs_preprocessed_routing {
                ...
                Some(
                    RoutingLoadContext::start_for_model(
                        client.clone(),
                        source,
            ...
            let migration_fallback: Option<Arc<dyn MigrationFallbackSource>> = Some(Arc::new(
                WorkerSetMigrationFallback::new(self.manager.clone(), card, spec.key.id()),
            ));
            ...
            let load_thresholds =
                LoadThresholdHandle::new(router_config.load_threshold_config.clone());
            let load_context = RoutingLoadContext::start_for_model(
                client,
                ...
                || card.model_type.supports_completions()
            {
                let worker_monitor = load_context
                    .monitor()
                    .expect("chat/completion load monitor")
                    .clone();
                ...
                let (text_kv_router, affinity) = if router_config.router_mode == RouterMode::KV {
                    ...
                    let affinity = {
                        let coordinator = create_affinity_coordinator(
                            affinity_ttl,
                            router_config.session_affinity_mode,
                            client.clone(),
                        )
                        .await?;
                        if let (Some(coordinator), Some(tracker)) = (&coordinator, scale_up) {
                            coordinator.enable_scale_up(tracker);
                        }
                        coordinator
                    };
