...
impl ModelManager {
    ...
    pub(crate) fn replace_discovery_group(
        ...
        for key in previous_member_keys.difference(&desired_member_keys) {
            self.remove_card_and_service_metrics(key);
        }
    ...
    fn remove_card_and_service_metrics(&self, key: &str) -> Option<ModelDeploymentCard> {
        let (_, card) = self.cards.remove(key)?;
        if let Ok(instance) = ModelCardInstanceId::from_path(key) {
            crate::http::service::worker_service::remove(card.name(), instance.instance_id);
        }
        Some((*card).clone())
    }
    ...
    pub fn remove_model_card(&self, key: &str) -> Option<ModelDeploymentCard> {
        let _reservation = self.reservation_lock.lock();
        let removed = self.remove_card_and_service_metrics(key);
        if removed.is_some() {
            self.publish_catalog_locked();
        }
        removed
