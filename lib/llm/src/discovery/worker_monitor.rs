...
impl WorkerLoadState {
    ...
    fn update_from_runtime_config(&mut self, runtime_config: &ModelRuntimeConfig) {
        ...
        self.declared_dp_ranks = Some((self.data_parallel_start_rank..dp_end).collect());
...
impl KvWorkerMonitor {
    ...
    pub(crate) fn with_remote_block_metrics(mut self, enabled: bool) -> Self {
        self.publish_remote_blocks = enabled;
        self
    }
...
impl WorkerLoadMonitor for KvWorkerMonitor {
    ...
    async fn start_monitoring(&self) -> anyhow::Result<()> {
        ...
        let publish_remote_blocks = self.publish_remote_blocks;
                ...
                tokio::select! {
                    ...
                    result = runtime_configs_rx.changed() => {
                        ...
                        for (lease_id, runtime_config) in runtime_configs.iter() {
                            ...
                            state.data_parallel_start_rank = runtime_config.data_parallel_start_rank;
                            state.data_parallel_size = runtime_config.data_parallel_size.max(1);
                    ...
                    kv_event = kv_event_future => {
                        ...
                        if let LoadObservation::Remote(load) = observation {
                            WORKER_LOAD_METRICS.observe_active_load(
                                worker.worker_id, worker.dp_rank, source.metric_label(),
                                if publish_remote_blocks { load.active_decode_blocks.or(load.kv_used_blocks) } else { None }, load.num_waiting_reqs,
                            );
                        }
                        ...
                        let (total_blocks, worker_overloaded) = {
                            ...
                            if let Some(admission_state) = &admission_state
                                && matches!(observation, LoadObservation::Remote(load) if load.num_waiting_reqs.is_some())
                            {
