...
对于生产环境的多节点集群，请参阅 [Kubernetes 部署指南](/dynamo/dev/kubernetes/getting-started/quickstart)。如需为开发从源码构建，请参阅[从源码构建](/dynamo/zh-CN/dev/advanced-customizations/building-from-source)。
...
如需查看包含后端框架版本在内的完整兼容性矩阵，请参阅[支持矩阵](/dynamo/dev/reference/compatibility)。
...
如需查看可用版本，请参阅[发布产物](/dynamo/dev/reference/release-artifacts)；
如需运行说明，请参阅各后端指南：[SGLang](/dynamo/dev/knowledge-base/modular-components/backends/sg-lang/overview) |
[TensorRT-LLM](/dynamo/dev/knowledge-base/modular-components/backends/tensor-rt-llm/overview) | [vLLM](/dynamo/dev/knowledge-base/modular-components/backends/v-llm/overview)
...
本指南使用 `--discovery-backend file`。如需设置 etcd，请参阅[服务发现](/dynamo/dev/knowledge-base/kubernetes/kubernetes-operator/service-discovery)。
...
请参阅 [Kubernetes 中的模型缓存](/dynamo/dev/knowledge-base/kubernetes/model-loading/model-caching#option-2-modelexpress-p2p-distribution)
    了解正确配置。
...
运行 `nvidia-smi` 检查你的驱动版本。Dynamo 对 CUDA 12 需要驱动 575.51.03+，对 CUDA 13 需要驱动 580.00.03+。B300/GB300 GPU 需要 CUDA 13。完整要求请参阅[支持矩阵](/dynamo/dev/reference/compatibility)。
    ...
    **容器运行但未检测到 GPU**
    ...
    # Wrong -- no GPU access
    docker run --network host --rm -it nvcr.io/nvidia/ai-dynamo/sglang-runtime:1.4.2
    ```
...
- [后端指南](/dynamo/dev/knowledge-base/modular-components/backends/sg-lang/overview) -- 后端特定配置和功能
- [分离式服务](/dynamo/dev/kubernetes/disaggregated-serving/overview) -- 独立扩展 prefill 和 decode
- [KV Cache 感知路由](/dynamo/dev/knowledge-base/modular-components/router/router-guide) -- 智能请求路由
- [Kubernetes 部署](/dynamo/dev/kubernetes/getting-started/quickstart) -- 生产环境多节点部署
