...
<Tip>想马上开始？请参阅 [Quickstart](/dynamo/zh-CN/dev/cli/getting-started/quickstart)，几分钟内即可安装并运行 Dynamo。</Tip>
...
<Note>
也提供包含所有依赖项的预构建容器。请参阅 [Release Artifacts](/dynamo/dev/reference/release-artifacts) 了解容器镜像。
</Note>
...
| 内存管理 | Dynamo KV Block Manager、[LMCache](/dynamo/dev/cli/kv-cache-offloading/overview)、[SGLang HiCache](/dynamo/dev/knowledge-base/modular-components/backends/sg-lang/hi-cache)、[FlexKV](/dynamo/dev/cli/kv-cache-offloading/overview) |
| 网络和存储 | Mooncake、DOCA NetIO、GDS、POSIX、S3、3FS（[通过 NIXL 支持](/dynamo/dev/knowledge-base/modular-components/kvbm/kvbm-design)） |
...
- [KV cache-aware routing](/dynamo/dev/knowledge-base/modular-components/router/router-design) 根据 worker 负载和现有缓存命中情况智能路由请求。通过复用预先计算的 KV 对，它绕过 prefill 计算，立即开始 decode 阶段。[Baseten](https://www.baseten.co/blog/how-baseten-achieved-2x-faster-inference-with-nvidia-dynamo/#how-baseten-uses-nvidia-dynamo) 应用 Dynamo KV 缓存感知路由后，在 Qwen3 Coder 480B A35B 上实现了 2 倍更快的 TTFT 和 1.6 倍吞吐量。

- [KV cache offloading](/dynamo/dev/knowledge-base/modular-components/kvbm/kvbm-design) 通过将 KV 缓存从 HBM 移动到主机内存、本地磁盘或远程存储等成本更低的存储层，扩展可用上下文窗口。复用预计算状态可改善 TTFT、降低总体拥有成本（TCO），并支持更长上下文处理。

- [分离式服务](/dynamo/zh-CN/dev/knowledge-base/concepts/system-architecture/disaggregated-serving) 在“设计原则”部分，我们介绍了分离式服务的概念。[InferenceX](https://newsletter.semianalysis.com/p/inferencex-v2-nvidia-blackwell-vs) 展示了它的性能。借助分离式服务和大规模专家并行，DeepSeek V3 可以达到约 7 倍吞吐量/GPU。
...
Dynamo 提供 [Planner](/dynamo/zh-CN/dev/knowledge-base/modular-components/planner/planner-design) 来规避这个问题。开发者只需用 TTFT 和 Time Per Output Token (TPOT) 设置 SLA。Planner 会检查在线流量，并自动决定如何扩展 prefill 和 decode worker，从而在维持指定 SLA 的同时有效处理流量峰值。

最近，Planner 已扩展到处理更复杂的场景，例如在相同 SLA 下 Input Sequence Length (ISL) 剧烈变化。请参阅 [Planner documentation](/dynamo/zh-CN/dev/knowledge-base/modular-components/planner/planner-guide) 了解更多详细信息。
...
Dynamo 提供内置指标、分布式追踪和日志，用于监控推理部署。请参阅 [Observability Guide](https://github.com/ai-dynamo/dynamo/blob/d778daf510ea14c296bfd8d88e58031d310d2943/docs/fern/pages/kubernetes/installation/observability.md) 了解设置详情。
...
- [KV Cache-Aware Routing](/dynamo/dev/knowledge-base/modular-components/router/router-guide) -- 配置智能请求路由
- [KV Cache Offloading](/dynamo/dev/knowledge-base/modular-components/kvbm/kvbm-guide) -- 设置多层内存管理
- [Planner](/dynamo/zh-CN/dev/knowledge-base/modular-components/planner/planner-guide) -- 配置基于 SLA 的自动扩缩容
- [Kubernetes Deployment](/dynamo/dev/kubernetes/getting-started/quickstart) -- 使用 Grove 进行大规模部署
- [Overall Architecture](/dynamo/zh-CN/dev/knowledge-base/overview) -- 完整技术设计
- [Support Matrix](/dynamo/dev/reference/compatibility) -- 检查硬件和引擎兼容性
