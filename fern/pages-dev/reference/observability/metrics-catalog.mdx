...
The standalone indexer exposes the device-tier-only counter `dynamo_kvrouter_kv_cache_events_applied` and the CKF counter as `dynamo_kvrouter_ckf_mutation_total`; see [Standalone KV Indexer](../../developer-guide/knowledge-base/modular-components/router/standalone-indexer.md).
