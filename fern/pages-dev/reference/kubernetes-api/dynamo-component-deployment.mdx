...
<Info>
  This reference covers user-configurable spec fields. For operator-injected defaults (images, ports, probes, resources) and environment variables, see the [Dynamo Operator](../../developer-guide/knowledge-base/kubernetes/kubernetes-operator/dynamo-operator.md) guide.
</Info>
...
<ParamField path="replicas" type="integer">
  Desired number of pods for this component. Minimum `0`. When `scalingAdapter` is set, this field is owned by the [DynamoGraphDeploymentScalingAdapter](../../developer-guide/knowledge-base/kubernetes/kubernetes-operator/autoscaling.md) and should not be modified directly.
</ParamField>
...
<ParamField path="multinode" type="MultinodeSpec">
  Configures a multinode component. See [Multinode Deployments](../../kubernetes/model-deployment/multinode-deployments.md).
</ParamField>
...
<ParamField path="scalingAdapter" type="ScalingAdapter">
  Opts the component into a [DynamoGraphDeploymentScalingAdapter](../../developer-guide/knowledge-base/kubernetes/kubernetes-operator/autoscaling.md) (DGDSA). Set it — even as an empty object, `scalingAdapter: {}` — to create a DGDSA that owns `replicas` so external autoscalers (HPA, KEDA, Planner) can drive scaling through the Scale subresource. Omit the field to opt out.
</ParamField>
...
<ParamField path="eppConfig" type="EPPConfig">
  EPP-specific configuration. Only valid when `type` is `epp`. Exactly one of `configMapRef` or `config` must be set. See the [Gateway API Routing Reference](../components/gateway-api-routing.mdx).
</ParamField>
...
<ParamField path="topologyConstraint" type="TopologyConstraint">
  Component-level topology placement. See [Topology-Aware Scheduling](../../developer-guide/knowledge-base/kubernetes/multinode/topology-aware-scheduling.md).
</ParamField>
...
<ParamField path="checkpoint" type="ComponentCheckpointConfig">
  Configures container-image snapshotting and restore for the component. Set `checkpoint.enabled: true` to opt in; omit `checkpointRef` for a DGD-managed automatic checkpoint, or set it to restore an existing [DynamoCheckpoint](../../developer-guide/knowledge-base/kubernetes/kubernetes-operator/snapshot.md).
</ParamField>
...
<CardGroup cols={2}>
  ...
  <Card title="Dynamo Operator" icon="gears" href="../../developer-guide/knowledge-base/kubernetes/kubernetes-operator/dynamo-operator.md">
