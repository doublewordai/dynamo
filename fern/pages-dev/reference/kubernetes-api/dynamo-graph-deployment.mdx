...
<ParamField path="priorityClassName" type="string">
  Name of the `PriorityClass` to use for Grove `PodCliqueSet`s. Requires the Grove pathway. See [Grove](../../developer-guide/knowledge-base/kubernetes/multinode/grove.md).
</ParamField>
...
<ParamField path="topologyConstraint" type="SpecTopologyConstraint">
  Deployment-level topology constraint. Components without their own `topologyConstraint` inherit this value. See [Topology-Aware Scheduling](../../developer-guide/knowledge-base/kubernetes/multinode/topology-aware-scheduling.md).
</ParamField>
...
<Indent>
  <ParamField path="kvTransferPolicy" type="KvTransferPolicy">
    Topology-aware routing for KV-cache transfers between prefill and decode workers. Set exactly one of `labelKey` or `clusterTopologyName`. See [Topology-Aware KV Transfer](../../developer-guide/knowledge-base/kubernetes/multinode/topology-aware-kv-transfer.md).
  </ParamField>
  <Indent>
...
<ParamField path="status.checkpoints" type="map[string]ComponentCheckpointStatus">
  Per-component checkpoint status, keyed by component name. See [Snapshot and Restore](../../developer-guide/knowledge-base/kubernetes/kubernetes-operator/snapshot.md).

  See: [ComponentCheckpointStatus](#componentcheckpointstatus)
</ParamField>

<ParamField path="status.rollingUpdate" type="RollingUpdateStatus">
  Progress of operator-managed rolling updates. Currently supported only for single-node, non-Grove deployments. See [Rolling Update](../../developer-guide/knowledge-base/kubernetes/kubernetes-operator/rolling-update.md).
</ParamField>
