...
Start the disaggregated service: See [Disaggregated Serving](../../../recipes/cli-templates/tensorrt-llm.mdx#disaggregated) to learn how to start the deployment.
...
- **Pre-built EFA image:** A multi-arch EFA image with the EFA SDK baked in is available on NGC:
...
- **Host-mount approach:** For custom images, use the standard `tensorrtllm-runtime` image and mount the EFA SDK from the host node:
...
<Info>`FI_EFA_ENABLE_SHM_TRANSFER` must be `0`. SHM transfers break NIXL GPU buffer registrations.</Info>
