...
For `/v1/chat/completions` and `/v1/completions`, Dynamo uses a non-empty
OpenAI request body `user` value as the router affinity ID when
`X-Dynamo-Session-ID` is absent. An explicit `X-Dynamo-Session-ID` always wins.
When both `user` and a supported agent-native session header are present,
`user` supplies routing affinity while the native header continues to supply
agent and tracing identity.
...
agent-aware consumers. Clients that need the same value for affinity and agent
identity should continue to send `X-Dynamo-Session-ID`.
