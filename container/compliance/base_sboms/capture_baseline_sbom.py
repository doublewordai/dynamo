...
Steps performed:
  ...
  4. syft scan the baseline only. Apply slim filter
      (drop properties/hashes/dependencies; keep evidence). Hard cap 12 MB.
...
_SIZE_CAP_BYTES = (
    12 * 1024 * 1024
)  # 12 MB cap accommodates large vendor baselines while bounding repository growth
...
def capture(
    ...
    if size > _SIZE_CAP_BYTES:
        logger.error(
            "Slim SBOM exceeds 12 MB cap (%.2f MB). "
