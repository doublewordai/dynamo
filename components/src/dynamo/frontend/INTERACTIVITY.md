...
Clients select a class using `x-dynamo-interactivity-pool` or `nvext.interactivity_pool`. Untagged requests use `default_pool`. Invalid/conflicting tags return HTTP 400. Requests can only use workers currently assigned to their home pool. There is no borrowing or fallback to another pool, including for explicit worker pins. If the home pool is full or draining, requests receive HTTP 503 even when another pool has spare capacity. Workers at or above the applicable budget are excluded. If no eligible worker/rank remains, the response is HTTP 503 with reason `interactivity_capacity`. Pool eligibility is intersected with Dynamo’s existing worker allow-list. Dynamo’s existing scheduler selects the DP rank; the pool policy adds no per-rank eligibility filter. Existing pinning, taints, KV scoring, engine limits, and cancellation handling remain in force.
...
Enable `DYN_LOG=info,dynamo_llm::kv_router::push_router::interactivity=debug` for decisions and state. Optional `DYN_FRONTEND_INTERACTIVITY_STATUS_PATH` writes a diagnostic JSON snapshot, with an endpoint suffix for multi-endpoint configurations. `occupied` and `effective_cap` in this snapshot are block counts; `kv_fraction` is active blocks divided by total KV capacity. `local_requests` only tracks stream ownership for draining and does not set capacity.
