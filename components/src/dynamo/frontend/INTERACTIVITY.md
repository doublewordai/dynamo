...
Clients select a class using `x-dynamo-interactivity-pool` or `nvext.interactivity_pool`. Untagged requests use `default_pool`. Invalid/conflicting tags return HTTP 400. Requests prefer home workers, then borrow other workers under the smaller class budget. A locally tracked borrowed stream keeps that smaller budget in effect until completion. Workers at or above the applicable budget are excluded. If no eligible worker/rank remains, the response is HTTP 503 with reason `interactivity_capacity`. Pool eligibility is intersected with Dynamo’s existing worker allow-list. Dynamo’s existing scheduler selects the DP rank; the pool policy adds no per-rank eligibility filter. Existing pinning, taints, KV scoring, engine limits, and cancellation handling remain in force.
