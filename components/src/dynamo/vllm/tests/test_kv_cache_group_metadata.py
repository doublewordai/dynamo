...
async def test_benchmark_gc_extension_is_recognised_without_importing_it(
    monkeypatch,
):
    """Importing dynamo.vllm.gc_policy starts the GC policy in the importing
    process, so the benchmark-mode extension must be matched by name."""
    monkeypatch.setitem(sys.modules, "dynamo.vllm.gc_policy", None)
    engine = _Engine(worker_groups=HYBRID_GROUPS)
    vllm_config = _vllm_config(FPM_GC_WORKER_EXTENSION_CLS)

    assert await configure_kv_event_block_size(engine, vllm_config) == 256
    assert engine.calls == [f"collective_rpc:{KV_CACHE_GROUP_METADATA_METHOD}"]


def test_benchmark_gc_extension_extends_dynamo_extension(monkeypatch):
    """vLLM has one extension slot: the benchmark-mode class must carry the
    KV cache group metadata method too."""
    monkeypatch.delenv("DYN_FPM_GC_POLICY", raising=False)
    module_name, _, class_name = FPM_GC_WORKER_EXTENSION_CLS.rpartition(".")
    extension_cls = getattr(importlib.import_module(module_name), class_name)

    assert issubclass(extension_cls, DynamoWorkerExtension)
    assert hasattr(extension_cls, KV_CACHE_GROUP_METADATA_METHOD)
