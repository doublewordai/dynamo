...
def _is_dynamo_worker_extension(qualname: str | None) -> bool:
    ...
    Dynamo's own classes are matched by name so that this check never imports
    ``dynamo.vllm.gc_policy``, whose import starts the GC policy in the
    importing process. Any other class is imported and checked for
    inheritance, so a user subclass keeps the worker-extension path.
    """
    if not qualname:
        return False
    if qualname in (DYNAMO_WORKER_EXTENSION_CLS, FPM_GC_WORKER_EXTENSION_CLS):
        return True
