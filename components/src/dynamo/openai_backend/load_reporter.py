...
class EngineLoadReporter:
    ...
    async def start(self) -> None:
        ...
        # Bootstrap every advertised rank so rank 0 is not the only target with
        # a usable report before the first metrics scrape.
        for dp_rank in range(self._data_parallel_size):
            self._publisher.publish(
                dp_rank, kv_used_blocks=0, num_waiting_reqs=0
            )
