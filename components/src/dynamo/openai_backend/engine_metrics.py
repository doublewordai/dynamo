...
``dynamo_namespace`` / ``dynamo_component`` / ``worker_id`` / ``model`` labels the
in-process backends inject.
"""
...
def engine_metrics_url(upstream_base_url: str) -> str:
    """Derive the engine's ``/metrics`` URL from its OpenAI base URL."""
...
class EngineMetricsCollector:
    ...
    def collect(self) -> Iterator:
        ...
        try:
            with urlopen(self._metrics_url, timeout=self._timeout) as response:
...
def register_engine_metrics(
    ...
    *,
    ...
    try:
        ...
        metrics_url = engine_metrics_url(upstream_base_url)
