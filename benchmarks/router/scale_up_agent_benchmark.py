...
"""Run a live, multi-turn Dynamo affinity scale-up experiment.
...
For a scale-up case it starts worker A and the frontend, launches BatchBench,
waits until every agent has completed the configured trigger turn, then starts
worker B while the remaining turns continue. The resulting JSONL event streams
and Dynamo structured logs are checked for the exact deterministic migration
cohort as well as worker/rank stickiness.
"""
...
SCENARIOS = ("a-only", "ab-start", "scale-up")
ROUTE_MESSAGES = {
    ...
    env.update(
        {
            "DYN_ROUTER_MIN_INITIAL_WORKERS": (
                "2" if case.scenario == "ab-start" else "1"
            ),
            "DYN_LOGGING_JSONL": "1",
...
def batchbench_command(args: argparse.Namespace, case: CaseConfig) -> ManagedCommand:
    events_path = case.output_dir / "agent-events.jsonl"
    results_path = case.output_dir / "batchbench-summary.csv"
    command = [
        args.batchbench_bin,
        "--model",
        args.served_model_name,
        ...
        args.model,
        "--host",
        f"http://127.0.0.1:{case.ports.frontend}",
        "--agents",
        str(args.agents),
        "--input-lognorm-median",
        str(args.input_median),
        "--input-lognorm-sigma",
        str(args.input_sigma),
        "--input-lognorm-max",
        str(args.input_max),
        "--output-lognorm-median",
        str(args.output_median),
        "--output-lognorm-sigma",
        str(args.output_sigma),
        "--output-lognorm-max",
        str(args.output_max),
        "--temperature",
        str(args.temperature),
        "--max-tokens-only",
        "--environment-lognorm-median",
        str(args.environment_median),
        "--environment-lognorm-sigma",
        str(args.environment_sigma),
        "--environment-lognorm-max",
        str(args.environment_max),
        "--tool-invocations",
        str(args.turns),
        "--barrier-after-invocation",
        str(args.scale_after_turn),
        "--tool-call-latency-lognorm-median-ms",
        str(args.tool_latency_median_ms),
        "--tool-call-latency-lognorm-sigma",
        str(args.tool_latency_sigma),
        "--tool-call-latency-lognorm-max-ms",
        str(args.tool_latency_max_ms),
        "--agent-header-template",
        "x-dynamo-session-id=scale-agent-{agent_id}",
        "--nvext-extra-field",
        "worker_id",
        "--events-jsonl",
        str(events_path),
    ...
    return ManagedCommand(
        name="batchbench",
        command=command,
        log_path=case.output_dir / "batchbench.log",
        cwd=args.repo,
        env=os.environ.copy(),
    )
...
def completed_trigger_agents(events_path: Path, trigger_turn: int) -> set[int]:
    return {
        int(record["agent_id"])
        for record in read_jsonl(events_path)
        if record.get("event") == "request_complete"
        and record.get("status") == "success"
        and record.get("invocation") == trigger_turn
        and isinstance(record.get("agent_id"), int)
    }
...
def run_workload(
    args: argparse.Namespace,
    case: CaseConfig,
    batchbench: ManagedCommand,
    worker_b: ManagedCommand,
    timeline_path: Path,
) -> None:
    events_path = case.output_dir / "agent-events.jsonl"
    batchbench.start()
    record_timeline(timeline_path, "batchbench_started", pid=batchbench.process.pid)
    deadline = time.monotonic() + args.workload_timeout
    worker_b_started = case.scenario == "ab-start"

    while True:
        return_code = batchbench.poll()
        if return_code is not None:
            if return_code != 0:
                raise RuntimeError(
                    f"BatchBench exited with status {return_code}; see {batchbench.log_path}"
                )
            break
        if time.monotonic() >= deadline:
            raise TimeoutError(
                f"BatchBench did not complete within {args.workload_timeout}s"
            )
        if case.scenario == "scale-up" and not worker_b_started:
            completed = completed_trigger_agents(events_path, args.scale_after_turn)
            if len(completed) == args.agents:
                record_timeline(
                    timeline_path,
    ...
    if case.scenario == "scale-up" and not worker_b_started:
        ...
        if failures:
            raise RuntimeError(
                f"{len(failures)} agent requests failed before the scale trigger"
            )
        raise RuntimeError(
            "workload ended before every agent completed the scale trigger turn"
        )
    record_timeline(timeline_path, "batchbench_completed")
    ...
    for session, session_routes in routes_by_session.items():
        ...
        if session in expected_migrations:
            if len(migration_indices) != 1:
                errors.append(
                    f"{session} has {len(migration_indices)} migrate decisions, expected one"
                )
                continue
            ...
            if before and target_key(before[0]) == target_key(after[0]):
                errors.append(f"{session} migration did not change its exact target")
            if (
                scale_event
            ...
            if len({target_key(route) for route in session_routes}) != 1:
                errors.append(f"{session} did not retain exact worker/rank affinity")
...
def run_case(
    ...
    batchbench = batchbench_command(args, case)
    nats = nats_command(args, case)
    processes = [
        process
        for process in (nats, worker_a, worker_b, frontend, batchbench)
    ...
    try:
        ...
        run_workload(args, case, batchbench, worker_b, timeline_path)
...
def parse_gpu_list(value: str) -> list[int]:
    ...
    if len(result) != 2 or len(set(result)) != 2 or any(gpu < 0 for gpu in result):
        raise argparse.ArgumentTypeError(
            "each worker must receive exactly two distinct GPUs"
        )
    return result
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--batchbench-bin", default="batchbench-agent")
    ...
    parser.add_argument("--input-median", type=float, default=256)
    parser.add_argument("--input-sigma", type=float, default=0.45)
    parser.add_argument("--input-max", type=int, default=512)
    parser.add_argument("--output-median", type=float, default=48)
    parser.add_argument("--output-sigma", type=float, default=0.35)
    parser.add_argument("--output-max", type=int, default=96)
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.0,
        help="Generation temperature for every BatchBench request",
    )
    parser.add_argument("--environment-median", type=float, default=96)
    parser.add_argument("--environment-sigma", type=float, default=0.45)
    parser.add_argument("--environment-max", type=int, default=192)
    parser.add_argument("--tool-latency-median-ms", type=float, default=400)
    parser.add_argument("--tool-latency-sigma", type=float, default=0.5)
    parser.add_argument("--tool-latency-max-ms", type=int, default=2000)
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    resolved_batchbench = shutil.which(args.batchbench_bin)
    if resolved_batchbench is None and not Path(args.batchbench_bin).is_file():
        raise ValueError(f"BatchBench executable not found: {args.batchbench_bin}")
    if resolved_batchbench is not None:
        args.batchbench_bin = resolved_batchbench
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    write_json(args.output_dir / "configuration.json", serializable_args(args))
